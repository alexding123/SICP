...
(define (install-polynomial-package)
  ...
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((result (reduce-terms (term-list p1) (term-list p2))))
          (list (tag (make-poly (variable p1) (car result))) (tag (make-poly (variable p1) (cadr result)))))
        (display "Polys are not in the samve var -- REDUCE-POLY" (list p1 p2))))
  ...
  (define (gcd-terms a b)
    (if (empty-termlist? b)
        (let* ((coeff-list (map cadr a)) 
          ...
          (div-all-terms-by-num a gcd-coeff))
  ...
  (define (pseudoremainder-terms a b)
   (let* ((o1 (order (first-term a))) 
          ...
          (divident (mul-term-by-all-terms (make-term 0 (fast-expt c (+ 1 (- o1 o2)))) a)))
     (cadr (div-terms divident b))))
  (define (reduce-terms n d)
    (let ((gcd (gcd-terms n d)))
      (list (car (div-terms n gcd)) (car (div-terms d gcd)))))
  ...
  (define (div-all-terms-by-num l num)
    (if (empty-termlist? l)
        (the-empty-termlist)
        (let ((t1 (first-term l)))
          (adjoin-term
           (make-term (order t1) (div (coeff t1) num))
           (div-all-terms-by-num (rest-terms l) num)))))
  ...
  (put 'reduce '(polynomial polynomial)
       (lambda (p1 p2)
         (let ((result (check-and-convert p1 p2)))
           (reduce-poly (car result) (cdr result))))) ; no need to tag cuz it's already done in reduce-poly
  (put 'make 'polynomial
...
(define poly2 (make-polynomial 'x '((2 11) (0 7))))
(define poly3 (make-polynomial 'x '((1 13) (0 5))))
(define poly4 (make-polynomial 'x '((1 1) (0 -1))))
(reduce poly1 poly4)