...
(define false 'false)
(define true 'true)
(define (literally-true? exp) (eq? exp true))
(define (clauses exp) (cdr exps))
(define (eval-and exp env)
  (define (expand-and seq)
    (let ((first-result (eval (first-exp seq) env)))
      (if (last-exp? seq)
          first-result
          (if (literally-true? first-result)
              (expand-and (rest-exps seq))
              false))))
  (if (null? (clauses seq))
      true
      (expand-and (clauses exp))))
(define (eval-or exp env)
  ...
  (if (null? (clauses seq))
      false
      (expand-or (clauses exp))))

;; or you can make them into nested ifs